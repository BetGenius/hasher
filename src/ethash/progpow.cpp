...
void hash(const epoch_context& context, int block_number, const hash256 *header_hash,
...
bool verify(const epoch_context& context, int block_number, const hash256 *header_hash,
    const hash256 &hashMix, uint64_t nonce, hash256 *hash_out) noexcept
{
    ...
    hash256 expected_hashMix;
    hash_mix(context, block_number, hash_seed, calculate_dataset_item_2048, &expected_hashMix);
